"""Fetch occurrence records for Finland, for one of the app's species.

Sources
  GBIF   (public, no key)  – all records with coordinates for the species' taxonKey
...
Output: ml/data/<species>/observations.csv (committed; contains only public data)

    python ml/ingest/fetch_observations.py                        # matsutake
    python ml/ingest/fetch_observations.py --species kanttarelli

The species key is the one js/species.js uses, so a mushroom is named the same thing in the
pipeline, in the data directory and in the app.
"""
...
# species key -> the same taxon in both registries. `laji` is None where the FinBIF identifier
# has not been established: api.laji.fi's taxonomy endpoints need a token, so it cannot be looked
# up without one, and guessing an identifier would silently fetch the wrong species. GBIF alone
# is the normal path anyway — the laji.fi pass only runs when LAJI_TOKEN is set. Fill this in
# (and re-run) when you have a token; nothing else needs to change.
TAXA = {
    "matsutake":   dict(gbif=5241820, laji="MX.72541", latin="Tricholoma matsutake"),
    "kanttarelli": dict(gbif=5249504, laji=None,       latin="Cantharellus cibarius"),
}
FIELDS = ["source", "id", "date", "year", "month", "day", "lat", "lon", "unc_m", "basis", "dataset", "dataset_key", "license", "locality", "remarks", "event_remarks", "habitat"]
...
def clean_text(s):
    """Free text from the observer (occurrenceRemarks / eventRemarks / habitat). GBIF records
    sourced from iNaturalist prefix occurrenceRemarks with an automated "Quality assessment: ..."
    tag joined by " | "; that tag is metadata about the record, not something the observer wrote,
    so it is stripped here rather than shown as if it were a remark."""
    s = (s or "").strip()
...
def gbif(taxon):
    rows, off = [], 0
    while True:
        q = dict(taxonKey=taxon, country="FI", hasCoordinate="true", hasGeospatialIssue="false",
...
def laji(token, taxon):
    ...
    rows, page = [], 1
    while True:
        q = dict(taxonId=taxon, countryId="ML.206", pageSize=1000, page=page, access_token=token)
        ...
        for r in j.get("results", []):
            ...
            rows.append(dict(source="laji", id=(u.get("unitId") or "").replace("http://", ""), date=d if ok else "",
                             ...
                             remarks=clean_text(u.get("notes") or g.get("notes")),
                             # The warehouse's default field set has no per-event or habitat notes
                             # (checked against 500+ live records); nothing to read here yet.
                             event_remarks=None, habitat=None))
...
def main():
    ...
    ap.add_argument("--species", default="matsutake", choices=sorted(TAXA))
    a = ap.parse_args()
    taxa = TAXA[a.species]
    out = os.path.join(ML, "data", a.species, "observations.csv")
    print(a.species, "=", taxa["latin"])

    rows = gbif(taxa["gbif"])
    print("gbif records:", len(rows))
    token = os.environ.get("LAJI_TOKEN")
    if token and not taxa["laji"]:
        print("LAJI_TOKEN is set but this species has no FinBIF taxon id — skipping laji.fi")
    if token and taxa["laji"]:
        lrows = laji(token, taxa["laji"])
